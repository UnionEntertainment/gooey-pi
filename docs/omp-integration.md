...
- Spawn for embedding: `omp --mode rpc --cwd <dir>` plus:
  ...
  - approvals: `--approval-mode <always-ask|write|yolo>` — only passed when the user overrides the default (settings page or the composer footer's Approval mode select); otherwise OMP's own `~/.omp/agent/config.yml` governs. There is no live approval RPC, so changing the override retires idle OMP runtimes and the next prompt respawns the session with the new flag.
  - extensions: `-e <path>` for each injected capability extension. GooeyPi injects scheduled-task, terminal-read, and session-collaboration extensions, plus the ask-user extension when the universal **Ask user** capability is enabled. No `--skill <path>` flag exists; the bundled `ego-browser` skill is injected through `--append-system-prompt <skillPath>` (which accepts a file path) when the **Browser | Ego Lite** capability is enabled, and the agent drives the separately installed ego lite browser via its `ego-browser` CLI.
...
- Capability bridges: `assets/extensions/omp-work-terminal.ts` exposes `terminal_read`, `terminal_open`, and `terminal_stop` through a scoped `CapabilityBridge` (default export `(pi) => void`, `pi.registerTool`) against the same loopback broker env contract. `terminal_open`/`terminal_stop` push `terminal:agent-open`/`terminal:agent-close` requests to the trusted renderer, which mounts or closes command tabs in the session's `TerminalDrawer` and replies on `terminal:agent-result`. Agent browser control moved to the separately installed ego lite browser: the bundled `ego-browser` skill is appended to the system prompt and the agent drives the user's real browser through the `ego-browser` CLI; the in-app Browser panel is a user-only preview surface. `assets/extensions/omp-work-schedules.ts` exposes list/create/update/manage tools through a separate scoped broker. `assets/extensions/omp-work-collaboration.ts` provides app-owned bounded list/models/create/read/send/wait tools for same-harness, same-working-directory top-level sessions.
