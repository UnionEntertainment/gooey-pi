...
describe('provider settings behavior and accessibility', () => {
  ...
  it('uses one bulk mutation and exposes the provider and model catalogue UI', async () => {
    ...
    const headers = [...container.querySelectorAll<HTMLButtonElement>('.provider-model-group__heading')]
    expect(headers).toHaveLength(2)
    expect(headers[0]?.textContent).toContain('ChatGPT Plus/Pro')
    expect(headers[0]?.textContent).toContain('1 of 2 on')
    expect(headers[1]?.textContent).toContain('Anthropic')
    expect(headers[1]?.textContent).toContain('0 of 1 on')
    const modelRows = () => [...container.querySelectorAll('.provider-model-row')]
    expect(modelRows()[0]?.textContent).toContain('GPT-5.6')
    expect(modelRows()[1]?.textContent).toContain('GPT-5.5')
    expect(modelRows()[2]?.textContent).toContain('Claude Sonnet')
    const openAiHeader = headers[0]!
    ...
    expect(modelRows()[0]?.textContent).toContain('Claude Sonnet')
    await click(openAiHeader)
    expect(openAiHeader.getAttribute('aria-expanded')).toBe('true')
